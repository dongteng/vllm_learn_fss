...
class CUDAGraphRunner:
    ...
    def capture(self,x):
        assert self.cuda_graph is None, 'Graph has already  been captured'
...
class ModelRunner:
    ...
    def capture_decode_graph(self):
        for batch in [1,2, 4, 8, 16,24, 32,40,48,56, 64,128]:
...
if __name__ == '__main__':
    ...
    #测试用例输入（先确定seq_len，再进行捕获）
    tokenizer = GPT2Tokenizer.from_pretrained('/data/nas/zhjm/model_cache/docker-shared/gpt2')
    input_text = "Hello, my name is Jim"
    ...
    #模拟 decode: 通常每步seq_len =1 ，这里可替换为input_ids[:,:1]
