...
class WorkerBase:
    """Worker interface that allows vLLM to cleanly separate implementations for                                            worker接口,用于让vllm能偶清晰地分离不同硬件平台的实现
    different hardware. Also abstracts control plane communication, e.g., to                                                同时,它还抽象了控制平面通信,例如用于向其他worker传递请求元数据(request metadata)
    ...
    def init_device(self) -> None:
        """Initialize device state, such as loading the model or other on-device                                            初始化设备相关状态,包括加载模型、分配显存等关键操作
        memory allocations.
        """
        raise NotImplementedError
    ...
    def reset_mm_cache(self) -> None:
        reset_fn = getattr(self.model_runner, "reset_mm_cache", None)                                                       #重置多模态(Multi-Modal)缓存,例如图像、视频等嵌入缓存
        if callable(reset_fn):
            reset_fn()

    def get_model(self) -> nn.Module: 
        raise NotImplementedError                                                                                           #获得当前模型的访问权限

    def apply_model(self, fn: Callable[[nn.Module], _R]) -> _R:
        """Apply a function on the model inside this worker."""                                                             #在当前Worker的模型上应用传入的函数,常用于模型参数修改等操作
    ...
    def sample_tokens(
        ...
        """Should be called immediately after execute_model iff it returned None.                                           执行采样(sampling)操作。仅当 execute_model() 返回 None 时才需要调用此方法。
        """
        raise NotImplementedError
...
class WorkerWrapperBase:
    """
    This class represents one process in an executor/engine. It is responsible                                              这个类表示执行器/引擎中的 一个进程。它负责延迟初始化 worker 并管理 worker 的生命周期。
    for lazily initializing the worker and handling the worker's lifecycle.
    We first instantiate the WorkerWrapper, which remembers the worker module                                               我们首先创建WorkerWrapper实例,它会记住worker模块和类名
    and class name. Then, when we call `update_environment_variables`, and the                                              然后当调用update_environment_variables时,真正的初始化工作在init_worker方法中完成
    ...
    def __init__(
        ...
        """
        Initialize the worker wrapper with the given vllm_config and rpc_rank.                                               使用给定的vllm_config和rpc_rank初始化worker wrapper
        Note: rpc_rank is the rank of the worker in the executor. In most cases,                                             注意:rpc_worker表示worker在executor的编号(rank) 在大多数情况下,它也等于worker在分布式组中的编号.但是当多个executor协同工作时,这两个编号可能不同
        it is also the rank of the worker in the distributed group. However,
        when multiple executors work together, they can be different.
        e.g. in the case of SPMD-style offline inference with TP=2,                                                          例如,在SPMD更风格的离线推理(tp=2)场景下,用户可以启动2个引擎/执行器. 每个执行器只有1个worker
        users can launch 2 engines/executors, each with only 1 worker.                                                       所有worker的rpc_rank=0,但它们在tp组中的rank是不同的
        All workers have rpc_rank=0, but they have different ranks in the TP
        group.
        """
        self.rpc_rank = rpc_rank
        self.global_rank = self.rpc_rank if global_rank is None else global_rank                                             #worker在整个分布式组中的编号,如果没有传就用rpc_rank
        self.worker: WorkerBase | None = None

        # do not store this `vllm_config`, `init_worker` will set the final                                                   不要在这里保存vllm_config,最终的配置会由init_worker设置
        # one.
        # TODO: investigate if we can remove this field in `WorkerWrapperBase`,                                               TODO:调研是否可以移除这个字段,init_cached_hf_modules可能已经不再需要了
    ...
    def adjust_rank(self, rank_mapping: dict[int, int]) -> None:
        """
        Adjust the rpc_rank based on the given mapping.                                                                         根据给定的映射关系调整rpc_rank
        It is only used during the initialization of the executor,                                                              这个函数仅在executor初始化阶段使用,用于在创建完所有的worker之后 重新调整各个worker的rpc_rank
    ...
    def init_worker(self, all_kwargs: list[dict[str, Any]]) -> None:
        ...
        if self.vllm_config.parallel_config.worker_extension_cls:                                                                #如果配置里还额外指定了一个扩展类(worker_extension_cls),例如parallel_config.worker_extension_cls="my_pkg.my_ext:MyWorkerExtension"
                                                                                                                                 #那么会动态把这个扩展类注入到worker_class中,目的给worker增加新的RPC接口/功能
            worker_extension_cls = resolve_obj_by_qualname(                                                                      #根据字符串动态解析python对象,例如"my_pkg.my_ext:MyWorkerExtension"-> <class MyWorkerExtension>
                self.vllm_config.parallel_config.worker_extension_cls
            )
            extended_calls = []
            if worker_extension_cls not in worker_class.__bases__:                                                               #避免重复继承,如果worker_extension_cls已经是基类之一 就不再注入
                ...
                worker_class.__bases__ = worker_class.__bases__ + (                                                              #动态修改类继承关系 原来worker_class.__bases__= = (BaseWorker,)
                    worker_extension_cls,                                                                                        #修改后= (BaseWorker, worker_extension_cls)
                )
                logger.info(
